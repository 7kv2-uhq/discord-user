...
import sys
init(autoreset=True)
__version__ = "Author: lcc 7KV2 3.5"
__github__= "https://github.com/7kv2-uhq"
...
def proc0():
    m_input = input(f"{Fore.LIGHTBLACK_EX}[{Fore.LIGHTGREEN_EX}7KV2{Fore.LIGHTBLACK_EX}]:> {Fore.LIGHTYELLOW_EX}").lower()
    if m_input=="exit":
        sys.exit(0)
    if m_input=="":
        proc0()
   ...
   if opt == 2:
    for username in usernames:
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{username}': {endpoint.json()['message']} |7KV2: Make sure you have a valid token.")
   elif opt == 1:
       body = {
           "username": usernames
       }
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{usernames}': {endpoint.json()['message']} |7KV2: Make sure you have a valid token.")
...
def ch_send_webhook(val0:str):
   if webhook_0 == True:
    ...
    try:
     webhook.post(
       username="7KV2",
       avatar_url="https://cdn.modrinth.com/data/sUnWuBk5/f113b53054821ea4197d8f03b9343610ac278d78.png",
       embeds=[
    {
      ...
      "thumbnail": {
        "url": "https://raw.githubusercontent.com/7kv2-uhq/discord-user/main/ascii-art-text-transparent.png"
      },
      "fields": [],
      "color": 16768000
    }
       ...
       print(f"{Lb}[!]{Fore.RED} Error: Something went wrong while sending the webhook request. Exception: {s} | 7KV2: Make sure you have a valid webhook URL")
   else:
      return
def opt1func(v1,v2):
   for i in range(v1):
